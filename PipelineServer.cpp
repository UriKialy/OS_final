...
void handleClientPipeline(int newSocket, Graph* graphPtr) {
    ...
    while (true) {
        ...
        switch (choice) {
                ...
                stage3.post([&]() {
                    ...
                    for (int i = 0; i < graphPtr->getNumVertices(); i++) {
                        for (int j = i + 1; j < graphPtr->getNumVertices(); j++) {
                            ...
                            longest = std::max(longest, static_cast<int>(path.size())); // Update longest path
                        }
                    }
                    std::string response = "Longest path in MST: " + std::to_string(longest) + "\n";
                    send(newSocket, response.c_str(), response.size(), 0);
                });
                break;
            }
            ...
            case 7: { // Get the average distance in the MST
                stage3.post([&]() {
                    if (!mstCreated) {
                        mst = MST(*graphPtr, "kruskal"); // Create the MST
                        mstCreated = true;
