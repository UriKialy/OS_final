    ...
    void processClient(int clientSocket, Graph* graphPtr) {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 2: { // Add an edge to the graph
                ...
                case 3: { // Remove an edge from the graph
                ...
                case 4: { // Get the total weight of the MST
                ...
                case 6: { // Get the shortest path in the MST
                    std::string prompt = "Provide the start and end vertices for the shortest path: ";
                    send(clientSocket, prompt.c_str(), prompt.size(), 0);
                ...
                case 7: { // Get the average distance in the MST (as an integer)
                    MST mst(*graphPtr, "kruskal");
                ...
                case 8: { // Print the MST (adjacency matrix format)
                    MST mst(*graphPtr, "kruskal");
                ...
                case 9: { // Exit the client connection
...
public:
    ...
    void workerLoop() {
        while (true) {
            ...
            {
                std::unique_lock<std::mutex> lock(queueMutex);
                cv.wait(lock, [this]() { return !tasks.empty() || stopFlag; });  // Wait for a task or stop signal
                if (stopFlag && tasks.empty()) return;  // Exit loop if stopFlag is set and no more tasks
                task = tasks.front();
